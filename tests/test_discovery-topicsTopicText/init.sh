...
while true; do
  ...
  if [[ $CONTINUE -eq 0 ]] && [[ $IS_KAFKA_POSTMAN_LOADED -gt 0 ]] && [ `grep -c 'sylvester' /tmp/data-poller-raw/sample_poller.log` -gt $MIN_RECORDS ] && [ `grep -c 'bugs_bunny' /tmp/data-poller-raw/sample_poller.log` -gt $MIN_RECORDS ] && [ `grep -c grep 'daffy_duck' /tmp/data-poller-raw/sample_poller.log` -gt $MIN_RECORDS ]; then
