...
while true; do
  ...
  if [[ $CONTINUE -eq 0 ]] && [[ $IS_KAFKA_POSTMAN_LOADED -gt 0 ]] && grep 'sylvester' /tmp/data-poller-raw/sample_poller.log>/dev/null 2>&1 && grep 'bugs_bunny' /tmp/data-poller-raw/sample_poller.log>/dev/null 2>&1 && grep 'porky_pig' /tmp/data-poller-raw/sample_poller.log>/dev/null 2>&1; then
